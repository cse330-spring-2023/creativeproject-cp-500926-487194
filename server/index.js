...
app.post("/api/newuser", (req, res) => {
    ...
    db.query("SELECT id FROM users WHERE id = ?", [id], (err, result) => {
      ...
      if (err) {
        ...
        db.query("INSERT INTO spotify (userId, displayName, song1, song2, song3, song4, song5) VALUES (?,?,?,?,?,?,?)",[userId, displayName,arr[0],arr[1],arr[2],arr[3],arr[4]],(err, result) => {
            if (err) {
              console.log(err);
              res.sendStatus(500);
            } else {
        ...
        db.query("UPDATE spotify SET song1 = ?, song2 = ?, song3 = ?, song4 = ?, song5 = ?", [arr[0], arr[1], arr[2], arr[3], arr[4]], (err, result) => {
          if (err){
            console.log(err);
            res.sendStatus(500);
          } else{
